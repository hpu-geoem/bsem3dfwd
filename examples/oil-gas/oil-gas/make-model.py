...
while True:
    ...
    for cell in tria.active_cells():
        ...
        if not (np.abs(c[0]) < 1000 and np.abs(c[1]) < 10000 and c[2] > 0 and c[2] < 5000):
            continue
...
for cell in tria.active_cells():
    ...
    if np.abs(c[0]) < 1000 and np.abs(c[1]) < 10000 and c[2] > 0 and c[2] < 5000:
        points = [c[1:]]
